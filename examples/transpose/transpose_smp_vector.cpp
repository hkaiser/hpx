...
///////////////////////////////////////////////////////////////////////////////
struct transpose_values
{
    template <typename Tuple>
    void operator()(Tuple& t) const
    {
        hpx::util::get<1>(t) = hpx::util::get<0>(t);
    }
};
...
int hpx_main(boost::program_options::variables_map& vm)
{
    ...
    using hpx::util::skip_iterator;
    using hpx::util::zip_iterator;
    using hpx::util::get;

    typedef hpx::vector<double>::iterator vector_iterator_type;
    typedef skip_iterator<vector_iterator_type> skip_iterator_type;
    typedef zip_iterator<vector_iterator_type, skip_iterator_type>
        zip_iterator_type;
        ...
        else
        {
            ...
            for_each(par, boost::begin(range), boost::end(range),
                [&](boost::uint64_t i)
                {
                    // for (boost::uint64_t j = 0; j < order; ++j)
                    // {
                    //    B[i + order * j] = A[order * i + j];
                    // }

                    vector_iterator_type begin_a = std::next(A.begin(), order * i);
                    vector_iterator_type end_a = std::next(begin_a, order);

                    vector_iterator_type begin_b = std::next(B.begin(), i);

                    zip_iterator_type begin(begin_a, skip_iterator_type(begin_b, order));
                    zip_iterator_type end(end_a, skip_iterator_type(B.end(), order));

                    for_each(par, begin, end, transpose_values());
                }
            );
        }
