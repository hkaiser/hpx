...
public:
    ...
    //-----------------------------------------------------
    hpx::lcos::future<bool> check_board_async(hpx::naming::id_type const& gid,
        ...
        std::vector<boost::uint8_t> const& board)
    {
        ...
        return hpx::async<action_type>(this->get_id(), board);
    }

    std::vector<boost::uint8_t> solve_board(
        std::vector<boost::uint8_t> const& board)
    {
        HPX_ASSERT(this->get_id());
        return solve_board_async(board).get();
    }
};
}
