...
namespace hpx { namespace parcelset { namespace policies { namespace libfabric {
    ...
    struct locality
    {
        ...
        static const uint32_t array_length =
            HPX_PARCELPORT_LIBFABRIC_LOCALITY_SIZE / 4;
        static const uint32_t array_size =
            HPX_PARCELPORT_LIBFABRIC_LOCALITY_SIZE;
...
#else
            throw fabric_error(
                0, "unsupported fabric provider, please fix ASAP");
#endif
        }
        ...
        {
#if defined(HPX_PARCELPORT_LIBFABRIC_LOCALITY_SOCKADDR)
            return reinterpret_cast<const struct sockaddr_in*>(&data)
                ->sin_addr.s_addr;
#elif defined(HPX_PARCELPORT_LIBFABRIC_GNI)
            return data[0];
#else
        ...
        {
            uint16_t port =
                256 * reinterpret_cast<const uint8_t*>(data_.data())[2] +
    ...
    private:
        friend bool operator==(locality const& lhs, locality const& rhs)
        {
            ...
            LOG_DEBUG_MSG(
                "Testing array equality " << ipaddress(a1) << ipaddress(a2));
#endif
