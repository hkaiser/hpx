...
{
public:
    explicit operation(bool& executed)
      : executed_(&executed)
    {
    }
...
{
public:
    explicit thrower(bool& executed)
      : executed_(&executed)
    {
    }
    ...
    thrower& operator=(thrower const&) = delete;
...
{
public:
    explicit foreach_func(int& count)
      : count_(&count)
    {
    }
    ...
    foreach_func(foreach_func const&) = delete;
    foreach_func(foreach_func&&) = default;
    foreach_func& operator=(foreach_func const&) = delete;
    foreach_func& operator=(foreach_func&&) = default;

    ~foreach_func() = default;
...
void success_test()
{
    ...
    {
        bool executed = false;
        HPX_TEST(execute_all(operation<int>(9, executed)) == 9);
        HPX_TEST(executed);
    }
    ...
    // Test returning an int with one cleanup operation
    {
        ...
        HPX_TEST(execute_all(operation<int>(9, executed),
                     operation<void>(cleaned_up)) == 9);
        HPX_TEST(executed && cleaned_up);
    }

    // Test returning void with one cleanup operation
    {
        bool executed = false, cleaned_up = false;
        execute_all(operation<void>(executed), operation<void>(cleaned_up));
        HPX_TEST(executed && cleaned_up);
    }
    ...
    // Test returning an int with two cleanup operations
    {
        ...
        HPX_TEST(execute_all(operation<int>(9, executed),
                     ...
                     operation<void>(cleaned_up2)) == 9);
        HPX_TEST(executed && cleaned_up1 && cleaned_up2);
    }

    // Test returning void with two cleanup operations
    {
        bool executed = false, cleaned_up1 = false, cleaned_up2 = false;
        execute_all(operation<void>(executed), operation<void>(cleaned_up1),
    ...
    // Test returning an int with three cleanup operations
    {
        bool executed = false, cleaned_up1 = false, cleaned_up2 = false,
             cleaned_up3 = false;
        HPX_TEST(execute_all(operation<int>(9, executed),
                     operation<void>(cleaned_up1), operation<void>(cleaned_up2),
                     operation<void>(cleaned_up3)) == 9);
        HPX_TEST(executed && cleaned_up1 && cleaned_up2 && cleaned_up3);
    }
    ...
    // Test returning void with three cleanup operations
    {
        bool executed = false, cleaned_up1 = false, cleaned_up2 = false,
             cleaned_up3 = false;
        execute_all(operation<void>(executed), operation<void>(cleaned_up1),
            operation<void>(cleaned_up2), operation<void>(cleaned_up3));
        HPX_TEST(executed && cleaned_up1 && cleaned_up2 && cleaned_up3);
    }
}
...
void operation_throws_test()
{
    ...
    {
        bool executed = false;
        HPX_TEST_THROW(execute_all(thrower<0>(executed)), error<0>);
        HPX_TEST(executed);
    }
    ...
    // Test primary operation throwing with one cleanup operation
    {
        bool executed = false, cleaned_up = false;
        HPX_TEST_THROW(
            execute_all(thrower<0>(executed), operation<void>(cleaned_up)),
            error<0>);
        HPX_TEST(executed && cleaned_up);
    }
    ...
    // Test primary operation throwing with two cleanup operations
    {
        bool executed = false, cleaned_up1 = false, cleaned_up2 = false;
        HPX_TEST_THROW(
            execute_all(thrower<0>(executed), operation<void>(cleaned_up1),
                operation<void>(cleaned_up2)),
            error<0>);
        HPX_TEST(executed && cleaned_up1 && cleaned_up2);
    }
...
void cleanup_throws_test()
{
    // Test single cleanup operation that throws
    {
        bool executed = false, cleaned_up = false;
        HPX_TEST_THROW(
            execute_all(operation<void>(executed), thrower<1>(cleaned_up)),
            error<1>);
        HPX_TEST(executed && cleaned_up);
    }
    ...
    // Test fist of two cleanup operations throwing
    {
        bool executed = false, cleaned_up1 = false, cleaned_up2 = false;
        HPX_TEST_THROW(
            execute_all(operation<void>(executed), thrower<1>(cleaned_up1),
                operation<void>(cleaned_up2)),
            error<1>);
        HPX_TEST(executed && cleaned_up1 && cleaned_up2);
    }
    ...
    // Test second of three cleanup operations throwing
    {
        ...
        HPX_TEST_THROW(
            execute_all(operation<void>(executed), operation<void>(cleaned_up1),
                thrower<2>(cleaned_up2), operation<void>(cleaned_up3)),
            error<2>);
        HPX_TEST(executed && cleaned_up1 && cleaned_up2 && cleaned_up3);
    }
...
void multiple_exceptions_test()
{
    // Test primary operation and cleanup operation throwing
    {
        bool executed = false, cleaned_up = false;
        HPX_TEST_THROW(
            execute_all(thrower<0>(executed), thrower<1>(cleaned_up)),
            error<0>);
        HPX_TEST(executed && cleaned_up);
    }
    ...
    // Test primary operation and first of two cleanup operations throwing
    {
        bool executed = false, cleaned_up1 = false, cleaned_up2 = false;
        HPX_TEST_THROW(
            execute_all(thrower<0>(executed), thrower<1>(cleaned_up1),
                operation<void>(cleaned_up2)),
            error<0>);
        HPX_TEST(executed && cleaned_up1 && cleaned_up2);
    }
    ...
    {
        bool executed = false, cleaned_up1 = false, cleaned_up2 = false;
        HPX_TEST_THROW(execute_all(operation<void>(executed),
                           thrower<1>(cleaned_up1), thrower<2>(cleaned_up2)),
            error<1>);
        HPX_TEST(executed && cleaned_up1 && cleaned_up2);
    }
