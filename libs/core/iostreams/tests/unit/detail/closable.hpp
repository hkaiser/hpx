...
namespace hpx::iostreams::test {
    ...
    public:
        closable_device(operation close_input, operation close_output)
          : close_input_(close_input)
          , close_output_(close_output)
        {
        }
    ...
    public:
        ...
        stream_offset seek(stream_offset, std::ios_base::seekdir)
        {
            return 0;
        }
        void close()
    ...
    public:
        ...
        {
            return {static_cast<char*>(nullptr), std::size_t(0)};
        }
    ...
    public:
        ...
        void close(Device&, std::ios_base::openmode which)
        {
            switch (which)
            {
            case std::ios_base::in:
    ...
    public:
        ...
        template <typename Device>
        stream_offset seek(Device&, stream_offset, std::ios_base::seekdir)
        {
            return 0;
        }
    ...
    public:
        typedef char char_type;
        closable_symmetric_filter(operation close)
          : close_(close)
        {
        }

        bool filter(char const*&, char const*, char*&, char*, bool)
        {
            return false;
        }
