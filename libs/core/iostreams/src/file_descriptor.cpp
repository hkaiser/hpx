...
#include <hpx/iostreams/device/file_descriptor.hpp>
...
namespace hpx::iostreams {
    ...
    namespace detail {
            ...
            std::filesystem::path const& p, std::ios_base::openmode const mode)
        {
            ...
            if (!(mode &
                ...
                ((mode & std::ios_base::trunc) &&
                    ((mode & std::ios_base::app) ||
                        !(mode & std::ios_base::out))))
            {
                throw std::ios_basefailure("bad open mode");
            }
            ...
            else
            {
                ...
                {
                    if (std::ios_baseTREAMS_FD_SEEK(fd, 0, SEEK_END) == -1)
                    {
                        ::close(fd);
                        throw system_failure("failed opening file");
                    }
...
#else
            int amt = std::ios_baseTREAMS_FD_WRITE(handle_, s, n);
...
#else
            if (off >
                    integer_traits<std::ios_baseTREAMS_FD_OFFSET>::const_max ||
                off < integer_traits<std::ios_baseTREAMS_FD_OFFSET>::const_min)
            {
                throw std::ios_base::failure("bad offset");
            }
            stream_offset result = std::ios_baseTREAMS_FD_SEEK(handle_,
                static_cast<std::ios_baseTREAMS_FD_OFFSET>(off),
    ...
    void file_descriptor_source::open(
        std::filesystem::path const& path, std::ios_base::openmode const mode)
    {
