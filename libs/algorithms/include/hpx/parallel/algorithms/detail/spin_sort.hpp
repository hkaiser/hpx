...
namespace hpx { namespace parallel { inline namespace v1 { namespace detail {
    ...
    ///                     if not make a recursive call swapping the ranges
    ...
    public:
        ...
        spin_sort_helper(
          ...
          : spin_sort_helper(
                first, last, comp, range_aux.begin(), range_aux.size())
        {
        }
        ...
        ~spin_sort_helper()
        {
            if (construct)
            {
                destroy(util::range<value_type*>(ptr, ptr + nptr));
                construct = false;
            }
      ...
      , owner(false)
    {
        ...
        if ((nlevel & 1) == 1)
        {
            ...
            range_it rng_a1(first, std::next(first, nelem_2));
            range_it rng_a2(std::next(first, nelem_2), last);
            ...
            range_buf rng_bx(
                rng_buf.begin(), std::next(rng_buf.begin(), nelem_2));
        ...
        else
        {
            ...
            range_it rng_a1(first, std::next(first, nelem_1));
            range_it rng_a2(std::next(first, nelem_1), last);
            ...
            rng_a1.last = std::next(rng_a1.begin(), rng_a2.size());
