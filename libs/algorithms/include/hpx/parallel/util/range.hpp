...
namespace hpx { namespace parallel { namespace util {
    ...
    struct range
    {
        ...
        bool valid() const
        {
            return parallel::v1::detail::distance(first, last) >= 0;
        }

        std::size_t size() const
        {
            return parallel::v1::detail::distance(first, last);
        }
    ...
    template <typename Iter, typename Sent>
    inline void destroy(range<Iter, Sent> r)
    {
        ...
        range<Iter1, Sent1> rng2, Compare cmp)
    {
        ...
        {
            *(rx1.begin()++) = (cmp(*rbx.begin(), *rx2.begin())) ?
                std::move(*(rbx.begin()++)) :
                std::move(*(rx2.begin()++));
        }
