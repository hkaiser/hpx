...
namespace hpx { namespace parallel { namespace util {
        ...
        Compare comp)
    {
        ...
        if (npos == 4 &&
            less_range(vrange_input[pos[2]].first, pos[2],
                vrange_input[pos[1]].first, pos[1], comp))
        {
            std::swap(pos[1], pos[2]);
        }
        ...
        if (npos == 4 &&
            less_range(vrange_input[pos[1]].first, pos[1],
                vrange_input[pos[0]].first, pos[0], comp))
        {
            std::swap(pos[0], pos[1]);
        }

        Iter1 it_dest = rdest.first;
        while (npos > 2)
        {
        ...
        range1_t raux1(rdest.first, it_dest);
        range1_t raux2(it_dest, rdest.last);
        if (pos[0] < pos[1])
        {
            return concat(raux1,
                full_merge(
        ...
        Compare comp)
    {
        ...
        if (nrange_input == 0)
        {
            return util::range<value_type*>(dest.first, dest.first);
        }
        if (nrange_input == 1)
        {
        ...
        util::range<value_type*> raux1(dest.first, it_dest);
        util::range<value_type*> raux2(it_dest, dest.last);
        if (pos[0] < pos[1])
        {
            return concat(raux1,
                uninit_full_merge(
