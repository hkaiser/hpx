//  Copyright (c) 2007-2017 Hartmut Kaiser
//
//  Distributed under the Boost Software License, Version 1.0. (See accompanying
//  file LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)

#if !defined(HPX_ASYNC_LAUNCH_POLICY_DISPATCH_NOV_26_2017_1243PM)
#define HPX_ASYNC_LAUNCH_POLICY_DISPATCH_NOV_26_2017_1243PM

#include <hpx/config.hpp>
#include <hpx/lcos/async_fwd.hpp>
#include <hpx/lcos/future.hpp>
#include <hpx/lcos/local/futures_factory.hpp>
#include <hpx/runtime/launch_policy.hpp>
#include <hpx/runtime/threads/thread_data_fwd.hpp>
#include <hpx/runtime/threads/thread_enums.hpp>
#include <hpx/runtime/threads/thread_helpers.hpp>
#include <hpx/traits/is_action.hpp>
#include <hpx/util/deferred_call.hpp>
#include <hpx/util/invoke.hpp>

#include <exception>
#include <type_traits>
#include <utility>

namespace hpx { namespace detail
{
    ///////////////////////////////////////////////////////////////////////////
    template <typename F, typename ...Ts>
    HPX_FORCEINLINE
    lcos::future<typename util::detail::invoke_deferred_result<F, Ts...>::type>
    call_sync(std::false_type, F f, Ts... vs) // decay-copy
    {
        using R = typename util::detail::invoke_deferred_result<F, Ts...>::type;
        try
        {
            return lcos::make_ready_future<R>(
                util::invoke(std::move(f), std::move(vs)...));
        }
        catch (...)
        {
            return lcos::make_exceptional_future<R>(
                std::current_exception());
        }
    }

    template <typename F, typename ...Ts>
    HPX_FORCEINLINE
    lcos::future<typename util::detail::invoke_deferred_result<F, Ts...>::type>
    call_sync(std::true_type, F f, Ts... vs) // decay-copy
    {
        try
        {
            util::invoke(std::move(f), std::move(vs)...);
            return lcos::make_ready_future();
        }
        catch (...)
        {
            return lcos::make_exceptional_future<void>(
                std::current_exception());
        }
    }

    ///////////////////////////////////////////////////////////////////////////
    template <typename Action>
    struct async_launch_policy_dispatch<Action,
        typename std::enable_if<
            !traits::is_action<Action>::value
        >::type>
    {
        ////////////////////////////////////////////////////////////////////////
        template <typename F, typename... Ts>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F, Ts...>::value,
            hpx::future<
                typename util::detail::invoke_deferred_result<F, Ts...>::type
            >
        >::type
        call(launch policy, F&& f, threads::thread_schedule_hint hint,
            Ts&&... ts)
        {
            using result_type =
                typename util::detail::invoke_deferred_result<F, Ts...>::type;

            if (policy == launch::sync)
            {
                using is_void = typename std::is_void<result_type>::type;
                return detail::call_sync(is_void{},
                    std::forward<F>(f), std::forward<Ts>(ts)...);
            }

            lcos::local::futures_factory<result_type()> p(
                util::deferred_call(std::forward<F>(f), std::forward<Ts>(ts)...));
            if (hpx::detail::has_async_policy(policy))
            {
                threads::thread_id_type tid = p.apply(policy, policy.priority(),
                    threads::thread_stacksize_default, hint);
                if (tid && policy == launch::fork)
                {
                    // make sure this thread is executed last
                    // yield_to
                    hpx::this_thread::suspend(threads::pending, tid,
                        "async_launch_policy_dispatch<fork>");
                }
            }
            return p.get_future();
        }

        template <typename F, typename T, typename... Ts>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F, T, Ts...>::value &&
           !std::is_same<
               typename std::decay<T>::type, threads::thread_schedule_hint
            >::value,
            hpx::future<
                typename util::detail::invoke_deferred_result<F, T, Ts...>::type
            >
        >::type
        call(launch policy, F&& f, T&& t, Ts&&... ts)
        {
            return call(policy, std::forward<F>(f),
                threads::thread_schedule_hint{}, std::forward<T>(t),
                std::forward<Ts>(ts)...);
        }

        template <typename F>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F>::value,
            hpx::future<typename util::detail::invoke_deferred_result<F>::type>
        >::type
        call(launch policy, F&& f)
        {
            return call(
                policy, std::forward<F>(f), threads::thread_schedule_hint{});
        }

        ////////////////////////////////////////////////////////////////////////
        template <typename F, typename... Ts>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F, Ts...>::value,
            hpx::future<
                typename util::detail::invoke_deferred_result<F, Ts...>::type
            >
        >::type
        call(hpx::detail::sync_policy, F&& f,
            threads::thread_schedule_hint hint, Ts&&... ts)
        {
            using result_type =
                typename util::detail::invoke_deferred_result<F, Ts...>::type;

            using is_void = typename std::is_void<result_type>::type;
            return detail::call_sync(is_void{},
                std::forward<F>(f), std::forward<Ts>(ts)...);
        }

        template <typename F, typename T, typename... Ts>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F, T, Ts...>::value &&
           !std::is_same<
               typename std::decay<T>::type, threads::thread_schedule_hint
            >::value,
            hpx::future<
                typename util::detail::invoke_deferred_result<F, T, Ts...>::type
            >
        >::type
        call(hpx::detail::sync_policy, F&& f, T&& t, Ts&&... ts)
        {
            return call(hpx::launch::sync, std::forward<F>(f),
                threads::thread_schedule_hint{}, std::forward<T>(t),
                std::forward<Ts>(ts)...);
        }

        template <typename F>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F>::value,
            hpx::future<typename util::detail::invoke_deferred_result<F>::type>
        >::type
        call(hpx::detail::sync_policy, F&& f)
        {
            return call(hpx::launch::sync, std::forward<F>(f),
                threads::thread_schedule_hint{});
        }

        ////////////////////////////////////////////////////////////////////////
        template <typename F, typename... Ts>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F, Ts...>::value,
            hpx::future<
                typename util::detail::invoke_deferred_result<F, Ts...>::type
            >
        >::type
        call(hpx::detail::async_policy policy,
            F&& f, threads::thread_schedule_hint hint, Ts&&... ts)
        {
            using result_type =
                typename util::detail::invoke_deferred_result<F, Ts...>::type;

            lcos::local::futures_factory<result_type()> p(
                util::deferred_call(std::forward<F>(f), std::forward<Ts>(ts)...));

            p.apply(policy, policy.priority(),
                threads::thread_stacksize_default, hint);
            return p.get_future();
        }

        template <typename F, typename T, typename... Ts>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F, T, Ts...>::value &&
           !std::is_same<
               typename std::decay<T>::type, threads::thread_schedule_hint
            >::value,
            hpx::future<
                typename util::detail::invoke_deferred_result<F, T, Ts...>::type
            >
        >::type
        call(hpx::detail::async_policy policy, F&& f, T&& t, Ts&&... ts)
        {
            return call(hpx::launch::async, std::forward<F>(f),
                threads::thread_schedule_hint{}, std::forward<T>(t),
                std::forward<Ts>(ts)...);
        }

        template <typename F>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F>::value,
            hpx::future<
                typename util::detail::invoke_deferred_result<F>::type
            >
        >::type
        call(hpx::detail::async_policy policy, F&& f)
        {
            return call(hpx::launch::async, std::forward<F>(f),
                threads::thread_schedule_hint{});
        }

        ////////////////////////////////////////////////////////////////////////
        template <typename F, typename... Ts>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F, Ts...>::value,
            hpx::future<
                typename util::detail::invoke_deferred_result<F, Ts...>::type
            >
        >::type
        call(hpx::detail::fork_policy policy,
            F&& f, threads::thread_schedule_hint hint, Ts&&... ts)
        {
            using result_type =
                typename util::detail::invoke_deferred_result<F, Ts...>::type;

            lcos::local::futures_factory<result_type()> p(
                util::deferred_call(std::forward<F>(f), std::forward<Ts>(ts)...));

            // make sure this thread is executed last
            threads::thread_id_type tid = p.apply(policy, policy.priority(),
                threads::thread_stacksize_default, hint);
            if (tid)
            {
                // yield_to
                hpx::this_thread::suspend(threads::pending, tid,
                    "async_launch_policy_dispatch<fork>");
            }
            return p.get_future();
        }

        template <typename F, typename T, typename... Ts>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F, T, Ts...>::value &&
           !std::is_same<
               typename std::decay<T>::type, threads::thread_schedule_hint
            >::value,
            hpx::future<
                typename util::detail::invoke_deferred_result<F, T, Ts...>::type
            >
        >::type
        call(hpx::detail::fork_policy policy, F&& f, T&& t, Ts&&... ts)
        {
            return call(hpx::launch::fork, std::forward<F>(f),
                threads::thread_schedule_hint{}, std::forward<T>(t),
                std::forward<Ts>(ts)...);
        }

        template <typename F>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F>::value,
            hpx::future<
                typename util::detail::invoke_deferred_result<F>::type
            >
        >::type
        call(hpx::detail::fork_policy policy, F&& f)
        {
            return call(hpx::launch::fork, std::forward<F>(f),
                threads::thread_schedule_hint{});
        }

        ////////////////////////////////////////////////////////////////////////
        template <typename F, typename T, typename... Ts>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F, Ts...>::value,
            hpx::future<
                typename util::detail::invoke_deferred_result<F, Ts...>::type
            >
        >::type
        call(hpx::detail::deferred_policy, F&& f,
            threads::thread_schedule_hint hint, Ts&&... ts)
        {
            using result_type =
                typename util::detail::invoke_deferred_result<F, Ts...>::type;

            lcos::local::futures_factory<result_type()> p(
                util::deferred_call(std::forward<F>(f), std::forward<Ts>(ts)...));

            return p.get_future();
        }

        template <typename F, typename T, typename... Ts>
        HPX_FORCEINLINE static
        typename std::enable_if<
            traits::detail::is_deferred_invocable<F, T, Ts...>::value &&
           !std::is_same<
               typename std::decay<T>::type, threads::thread_schedule_hint
            >::value,
            hpx::future<
                typename util::detail::invoke_deferred_result<F, T, Ts...>::type
            >
        >::type
        call(hpx::detail::deferred_policy policy, F&& f, T&& t, Ts&&... ts)
        {
            return call(hpx::launch::deferred, std::forward<F>(f),
                threads::thread_schedule_hint{}, std::forward<T>(t),
                std::forward<Ts>(ts)...);
        }

        template <typename F>
        HPX_FORCEINLINE static typename std::enable_if<
            traits::detail::is_deferred_invocable<F>::value,
            hpx::future<
                typename util::detail::invoke_deferred_result<F>::type
            >
        >::type
        call(hpx::detail::deferred_policy policy, F&& f)
        {
            return call(hpx::launch::deferred, std::forward<F>(f),
                threads::thread_schedule_hint{});
        }
    };
}}

#endif
