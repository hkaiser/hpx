...
namespace hpx { namespace parallel { HPX_INLINE_NAMESPACE(v1)
{
    ...
    namespace detail
    {
        ...
        {
            // we care about the first part of the pair only
            return std::make_pair(lhs.first + rhs.first, 0);
        }

        template <typename FwdIter, typename OutIter, typename Op>
        typename std::iterator_traits<FwdIter>::value_type
        sequential_exclusive_scan(FwdIter first, FwdIter last, OutIter dest, F && op)
        {
                ...
                return util::partitioner<ExPolicy, OutIter, pair_type>::
                    call_with_index(
                        ...
                        hpx::util::unwrapped(
                            ...
                            {
                                return copy_if_helper(policy, std::move(r),
